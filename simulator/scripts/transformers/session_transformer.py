...
def transform_row_to_session(
    ...
    energy_cents = None
    idle_cents = None
    if status in ["ACTIVE", "COMPLETED"] and energy_delivered_kwh is not None:
        energy_cents = int(round(energy_delivered_kwh * price_cents_per_kwh))

        idle_cents = 0
        if status == "COMPLETED" and connector_power is not None and connector_power > 0:
        ...
        total_cents = int((energy_cents or 0) + (idle_cents or 0))
